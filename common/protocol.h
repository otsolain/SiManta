...
constexpr int      TIMEOUT_MS         = 30000;   // 30 seconds (balance between wake-from-sleep recovery and stale connections)
constexpr int      DEFAULT_CAPTURE_MS = 1500;    // ~0.67 fps default (scales to 30+ students on shared WiFi)
constexpr int      DEFAULT_QUALITY    = 60;      // JPEG quality (good balance of size vs clarity)
constexpr double   DEFAULT_SCALE      = 0.5;     // Half-res thumbnails by default
...
enum class MsgType : uint8_t {
    HELLO        = 0x01,   // student â†' teacher: registration
    FRAME        = 0x02,   // student â†' teacher: screenshot JPEG (full frame)
    ACK          = 0x03,   // teacher â†' student: acknowledgment
    PING         = 0x04,   // bidirectional: keepalive
    PONG         = 0x05,   // bidirectional: keepalive response
    FRAME_DELTA  = 0x06,   // student â†' teacher: delta frame (changed tiles only)
    QUALITY_HIGH = 0x07,   // teacher â†' student: switch to high-res mode (fullscreen view)
    QUALITY_NORMAL = 0x08, // teacher â†' student: switch to normal mode (thumbnail view)
    SET_UPDATE_SPEED = 0x09, // teacher â†' student: set screenshot capture interval
    CMD          = 0x10,   // teacher â†' student: command (future)
    MESSAGE      = 0x11,   // teacher â†' student: text message popup
    LOCK_SCREEN  = 0x12,   // teacher â†' student: lock student screen
    UNLOCK_SCREEN= 0x13,   // teacher â†' student: unlock student screen
    SEND_URL     = 0x14,   // teacher â†' student: open URL in browser
    CHAT_MSG     = 0x15,   // bidirectional: chat message
    HELP_REQUEST = 0x16,   // student â†' teacher: request help
    HELP_CLEAR   = 0x17,   // teacher â†' student: clear help request
    APP_STATUS   = 0x18,   // student â†' teacher: active app info + CPU/RAM
    TRANSFER_START = 0x20, // teacher â†' student: file transfer begin
    TRANSFER_CHUNK = 0x21, // teacher â†' student: file data chunk
    TRANSFER_END   = 0x22, // teacher â†' student: file transfer complete
    DIR_LIST_REQUEST  = 0x30, // teacher â†' student: list directory contents
    DIR_LIST_RESPONSE = 0x31, // student â†' teacher: directory listing
    MKDIR_REQUEST     = 0x32, // teacher â†' student: create folder
    FILE_RETRIEVE_REQUEST = 0x33, // teacher â†' student: request file download
    FILE_RETRIEVE_START   = 0x34, // student â†' teacher: file retrieval begin
    FILE_RETRIEVE_CHUNK   = 0x35, // student â†' teacher: file retrieval data
    FILE_RETRIEVE_END     = 0x36, // student â†' teacher: file retrieval complete
    DEMO_START   = 0x40,   // teacher â†' student: start demo mode (teacher screen broadcast)
    DEMO_FRAME   = 0x41,   // teacher â†' student: demo screen frame
    DEMO_STOP    = 0x42,   // teacher â†' student: stop demo mode
    WAKE_ON_LAN  = 0x50,   // teacher internal: wake student PC via magic packet
    KICK         = 0x60,   // teacher â†' student: force disconnect (stop reconnecting)
};
struct PacketHeader {
